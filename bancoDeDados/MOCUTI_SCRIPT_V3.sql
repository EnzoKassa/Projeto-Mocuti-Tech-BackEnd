...
c.tipo_cargo, cc.id_canal_comunicacao, cc.tipo_canal_comunicacao
	...
	CREATE OR REPLACE VIEW mocuti.eventos_usuario AS
	SELECT
		e.id_evento,
		e.nome_evento,
	...
	select * from mocuti.eventos_usuario where id_usuario=1;

	CREATE OR REPLACE VIEW vw_publico_alvo_genero AS
	SELECT genero, COUNT(*) AS total, ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM usuario), 2) AS percentual
	FROM usuario
	GROUP BY genero;

	select * from vw_publico_alvo_genero;

	CREATE OR REPLACE VIEW vw_faixa_etaria_usuarios_ativos AS
	SELECT COUNT(*) AS total_ativos,
		SUM(CASE WHEN TIMESTAMPDIFF(YEAR, dt_nasc, CURDATE()) <= 15 THEN 1 ELSE 0 END) AS ate_15_anos,
