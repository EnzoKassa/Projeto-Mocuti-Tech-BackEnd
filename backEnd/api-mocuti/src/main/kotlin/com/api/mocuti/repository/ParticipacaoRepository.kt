package com.api.mocuti.repository
            ...
            interface ParticipacaoRepository : JpaRepository<Participacao, ParticipacaoId> {
                ...
                fun listarParticipacoesPassadas(
                    @Param("usuarioId") usuarioId: Int,
                    @Param("diaLimite") diaLimite: LocalDate
                ): List<Participacao>
                ...
                @Query("""
                    SELECT p 
                    FROM Participacao p 
                    WHERE p.evento.idEvento = :idEvento AND p.isInscrito = true
                """)
                fun findByEventoIdAndIsInscritoTrue(@Param("idEvento") idEvento: Int): List<Participacao>

                @Query("SELECT p FROM Participacao p WHERE p.evento.idEvento = :idEvento")
                fun findByEventoId(idEvento: Int): List<Participacao>
